...
# the ABI-relevant public stack: Ubuntu 22.04 + CUDA 12.4 devel + CPython 3.10 +
...
def build_wheelhouse(force_rebuild: bool = False) -> dict[str, object]:
    ...
    cache_root = pathlib.Path("/cache/trellis2") / key
...
def validate_and_package(cache_key: str) -> dict[str, object]:
    ...
    build_cache.reload()
    cache_root = pathlib.Path("/cache/trellis2") / cache_key
    ...
    if response.status_code == 404:
        response = requests.post(
            ...
            json={
                ...
                "body": (
                    "Prebuilt TRELLIS.2 dependencies for Ubuntu 22.04, Python 3.10, "
